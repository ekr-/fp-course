...
tok p = (\r -> (spaces >>> valueParser r) ) =<< p  
...
betweenCharTok o c p = between (charTok o) (charTok c) p
...
-- True
hex :: Parser Char
hex = let hInt s = case readHex s of Full a -> a; Empty -> 0 in
    do
        v <- replicateA 4 (satisfy isHexDigit)
        return (chr (hInt v))
