...
natural :: Parser Int
natural = bindParser (\k -> case read k of Empty  -> constantParser (UnexpectedString k)
...
bindParser f (P pa) = P (\i -> case (pa i) of Result i' a -> parse (f a) i'
...
(>>>) (P pa) (P pb) = P (\i -> case pa i of Result i' _ -> pb i'
                                            UnexpectedEof -> UnexpectedEof
                                            ExpectedEof i'' -> ExpectedEof i''
                                            UnexpectedChar c -> UnexpectedChar c
                                            UnexpectedString s -> UnexpectedString s)
...
(P p1) ||| (P p2) = P (\i -> case p1 i of Result i' a -> Result i' a
                                          _ -> p2 i)
...
list1 k = bindParser (\k' -> bindParser (\kk' -> valueParser (k' :. kk')) (list k)) k
...
sequenceParser l = foldRight (\a b -> bindParser (\k -> bindParser (\kk -> valueParser (k :. kk)) b) a) (valueParser Nil) l
...
firstNameParser = bindParser (\k -> bindParser (\kk -> valueParser (k :. kk)) (list lower)) upper
...
surnameParser = bindParser (\k -> bindParser (\kk -> bindParser (\kk' -> valueParser (k :. kk ++ kk')) (list lower)) (thisMany 5 lower)) upper
...
phoneParser = bindParser (\d -> bindParser (\k -> bindParser (\k' -> valueParser (d :. k ++ (k':.Nil))) (is '#')) phoneBodyParser) digit
...
personParser = 
        ...
        bindParser (\sn ->
        bindParser (\sm ->
        bindParser (\p -> valueParser (Person a fn sn sm p)) (spaces1 >>> phoneParser)) (spaces1 >>> smokerParser)) (spaces1 >>> surnameParser)) (spaces1 >>> firstNameParser)) ageParser
...
instance Functor Parser where
  ...
  f <$> k = bindParser (\a -> valueParser (f a)) k
