...
instance Monad f => Applicative (StateT s f) where
  ...
  (StateT f) <*> (StateT k) = StateT (\s -> f s >>= \(f', t) -> (\(a,b) -> (f' a, b)) <$> k t) 
...
instance Monad f => Monad (StateT s f) where
  ...
  f =<< (StateT k) = StateT (\s -> k s >>= \(a, t) -> runStateT (f a) t)
...
state' k = StateT (\s -> ExactlyOne $ k s)
...
distinct' lst = fst $ runState' (filtering p lst) S.empty where
    p :: Ord a => a -> State' (S.Set a) Bool
    p a = state' (\s -> if S.member a s then (False, s) else (True, S.insert a s))
...
distinctF lst = fst <$> runStateT (filtering p lst) S.empty where
    p :: (Ord a, Num a) => a -> StateT (S.Set a) Optional Bool
    p a = StateT (\s -> if a > 100 then Empty else if S.member a s then Full (False, s) else Full (True, S.insert a s))
...
instance Applicative f => Applicative (OptionalT f) where
  ...
  OptionalT f <*> OptionalT fa = OptionalT (lift2 (<*>) f fa)
...
instance Monad f => Monad (OptionalT f) where
  f =<< (OptionalT fa) = OptionalT (fa >>= \x -> case x of
...
instance Applicative (Logger l) where
  pure = Logger Nil
  Logger lst f <*> Logger lst' a = Logger (lst ++ lst') (f a)
...
instance Monad (Logger l) where
  f =<< Logger lst a = let (Logger lst' b) = f a in Logger (lst ++ lst') b
