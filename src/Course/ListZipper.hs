...
findLeft f (ListZipper l a r) = let p = break f l in case p of
    (_, Nil) -> IsNotZ
...
findRight f (ListZipper l a r) = let p = break f r in case p of
    (_, Nil) -> IsNotZ
...
moveRightLoop (ListZipper l a (h:.t)) = ListZipper t h (a:.l)
...
nth n (ListZipper l a r)
    | n < length l = moveLeftN (length l - n) (ListZipper l a r)
    | n == length l = IsZ (ListZipper l a r)
    | otherwise = if (n - length l) > length r then IsNotZ else moveRightN (n - length l) (ListZipper l a r) 
...
end (ListZipper l a r) = let (h:.t) = reverse r in ListZipper (t ++ (a:.l)) h Nil
...
start (ListZipper Nil a r) = ListZipper Nil a r
start (ListZipper l a r) = let (h:.t) = reverse l in ListZipper Nil h (t ++ (a:.r))
...
instance Extend ListZipper where
  f <<= z = ListZipper (unfoldr ((<$>) (\z' -> (f z', z')) . toOptional . moveLeft) z) (f z) (unfoldr ((<$>) (\z' -> (f z', z')) . toOptional . moveRight) z)
