...
instance Monad (State s) where
  ...
  f =<< State a = State (\s -> let (g, t) = a s in runState (f g) t)
...
findM f (x:.xs) = f x >>= (\q -> if q then return (Full x) else findM f xs) 
...
firstRepeat lst = fst $ runState (findM p lst) S.empty where
    p :: Ord a => a -> State (S.Set a) Bool
    p a = State (\s -> if S.member a s then (True, s) else (False, S.insert a s))  
...
sumDigits 0 = 0
sumDigits x = sumDigits (x `div` 10) + let d = x `mod` 10 in d*d 
